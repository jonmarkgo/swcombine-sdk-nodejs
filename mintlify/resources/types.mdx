...
<ParamField path="class" type="string">
  ...
  <Warning>
    Class ids were renumbered by the SW Combine API in a server-side sync. Ship class ids
    shifted by +1 (e.g. Super Capital `0 → 1`, Capital Ship `1 → 2`) and vehicle class ids
    by +21 (e.g. Ground Vehicles `0 → 21`, Speeders `1 → 22`). The SDK passes the value
    through unchanged, so if you cached numeric class ids, update them to the new numbering.
    Filtering by class name is unaffected.
  </Warning>
</ParamField>
